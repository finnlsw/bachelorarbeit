...
for i, fits_file in enumerate(lightFiles):
    ...
    print(f"file{i} median is:{np.median(corrected_image)}")
    print(f"exptime={exptime}, nearest dark={nearest_exptime_master_dark}")
    print(f"mean dark={np.mean(masterDarkFrame)}, *corrfactor: {np.mean(masterDarkFrame)*correction_factor}")
