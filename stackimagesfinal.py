...
input_files = glob.glob(os.path.join(input_path, 'lightCor_HIP100587_B (Johnson)_*.fit'))
...
# XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx R filter fehlt noch
exptimeList=[2.0, 5.0, 10.0, 30.0, 60.0]
for exptime in exptimeList:
    stackedImage, stackedHeader = stack_images(input_files, exptime)
    save_image(output_path, stackedImage, stackedHeader, custom_text=f"mesh_8_images_stacked")
print("finished")

"""
batch_size = 10
for i in range(0, len(input_files), batch_size):
    stackedImage, stackedHeader = stack_images(input_files[i:i+batch_size], 40.0)
